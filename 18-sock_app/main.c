...
static void run_application(const char *basename, char **args, int n)
{
	...
	if (strcmp(args[0], "server") == 0) {
		...
		skaddr = (struct sock_addr*)malloc(sizeof(struct sock_addr)*3);
		skaddr[0].ip = inet_addr(args[1]);
		skaddr[0].port = htons(atoi(args[2]));
		skaddr[1].ip = inet_addr(args[3]);
		skaddr[1].port = htons(atoi(args[4]));
